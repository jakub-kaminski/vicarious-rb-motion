...
void Vicarious::GripFKSolver::solve(std::vector<double> jawAngle) {
   double j8pos = chain->genericTreeMap["arm_link_8"]->jState.pos;
   double jaw8a = j8pos - (jawAngle[0] * 0.5);
   double jaw8b = j8pos + (jawAngle[0] * 0.5);
   chain->genericTreeMap["arm_link_8a"]->jState.pos = jaw8a;
   chain->genericTreeMap["arm_link_8b"]->jState.pos = jaw8b;
   updateLinksWithChildren("arm_link_7");
}

std::vector<double> Vicarious::CameraIKSolver::solve(Eigen::Matrix4d leftEff, Eigen::Matrix4d rightEff) {
    Eigen::Vector4d lookAt = (leftEff.block(0,3,4,1) + rightEff.block(0,3,4,1)) * 0.5;
    lookAt[2] += 0.020;
    Eigen::Vector4d lookAtTransf = cameraRefT.inverse() * lookAt;
    double angle3 = atan2(lookAtTransf[2], lookAtTransf[0]);
    double angle4 = atan2(lookAtTransf[1], lookAtTransf[0]);
    return {angle3, angle4};
}
